...
当前版本基于一份标准订单完整快照判断异常，已经完成处置闭环；下一步不会只是继续增加简单规则，而是接入订单、仓储和物流事件，识别单个系统难以发现的状态不一致。
...
物流系统：没有揽收或轨迹长时间未更新
                         ↓
统一关联订单与证据 → 判断卡点和风险等级 → 通知对应负责人 → 跟踪到恢复
```

计划按以下顺序持续迭代：

1. 定义 ERP / OMS、WMS 和物流事件契约及适配器边界；
2. 按订单号和事件时间线聚合多来源状态，保留每条判断证据；
3. 增加“已付款未出库”“已出库未揽收”“物流停滞”等跨系统规则；
4. 根据卡点把任务路由给订单运营、仓库或物流负责人；
5. 使用企业授权的脱敏数据校准 SLA、误报率和通知策略。

这部分属于明确的后续路线图，当前仓库尚未内置具体企业的 WMS、物流平台连接器，也不把规划中的能力描述为已经交付。

...
成熟 ERP 通常已经提供订单审核、异常拦截和库存预警。本项目不计划复制整套 ERP，而是在企业不更换现有系统的前提下，补充它难以覆盖的自定义协同流程：

- 将现有 ERP、WMS、物流或自研系统输出的必要事件统一成一份证据时间线；
- 用企业自己的 SLA 和跨系统规则判断卡点，而不是受限于单一产品的预置规则；
- 只把需要协作的异常送入飞书，按卡点路由给订单运营、仓库或物流负责人；
- 将接入、规则、审计和通知保留在可独立部署的服务中，避免为增加一条流程替换整套业务系统。

这种补充层只在现有系统存在流程缺口时有价值；如果企业使用的 ERP 已经完整覆盖同一套识别、处置和审计流程，应直接复用现有能力。

### 执行配额与成本边界

当前实现由自托管的 n8n、FastAPI 和 PostgreSQL 执行规则，通过飞书 OpenAPI 更新多维表格，并通过群机器人 Webhook 发送通知。它**没有创建或触发飞书多维表格原生自动化流程，因此不消耗多维表格的每月自动化运行次数**。

飞书侧仍有独立的 OpenAPI 与机器人频控。自定义机器人当前限制为单租户、单机器人 `100 次/分钟`、`5 次/秒`；多维表格写入也需要控制并发并处理限流。系统使用 Outbox 重试和通知降噪，避免把瞬时外部失败扩散到订单事务。具体限制以[飞书开放平台频控策略](https://open.feishu.cn/document/server-docs/api-call-guide/frequency-control?lang=zh-CN)为准。

本仓库本身不按订单量或工作流执行次数收费，但“可自托管”不等于“零成本”：企业仍需承担服务器、监控、备份、运维以及真实平台适配器的开发维护成本。它也不能替代企业本来就需要的 ERP、WMS 和物流服务。是否比购买商业模块更经济，应按实际报价、订单规模和长期维护成本评估，而不是只比较软件授权费。
